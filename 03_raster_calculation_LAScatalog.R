...
path_points  <- paste0(basedir, "/4_Daten/points/day4.laz") # input
path_area    <- paste0(basedir, "/4_Daten/sites/convex/area_polygons.shp") # input
path_rasters <- paste0(basedir, "/4_Daten/rasters_", raster_resolution * 100, "cm") # output
...
for (area_ID in area_IDs) {
  ...
  # execute
  dtm_ctg.LAScatalog(
    ctg_retiled, paste0(path_rasters, "/DTM"),
...
for (area_ID in area_IDs) {
  ...
  opt_output_files(ctg_retiled) <- paste0(
    dirname(path_points), "/02_normalized/area_",
    area_ID, "_norm_{ID}"
  )
